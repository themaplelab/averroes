...
public class Tests {
	...
	/**
	 * Perform content-based comparison between JSON objects that represent the
	 * generated and the expected classes.
	 * 
	 * @param testCase
	 * @param analysis
	 */
	public static void compareJson(String testCase, String analysis) {
		Collection<File> generatedFiles = CommonOptions.findJsonFiles(testCase, analysis, PrinterType.GENERATED);
		Collection<File> expectedFiles = CommonOptions.findJsonFiles(testCase, analysis, PrinterType.EXPECTED);
		...
		generatedFiles.stream().filter(f -> !f.getPath().endsWith(xtaJson) && !f.getPath().endsWith(rtaJson))
				.forEach(generatedJsonFile -> {
					File expectedJsonFile = expectedFiles.stream()
							.filter(f -> f.getName().equals(generatedJsonFile.getName())).findFirst().get();
					...
					try {
						...
						if (!generatedSootClass.isEquivalentTo(expectedSootClass)) {
							throw new AssertionError("The contents of " + generatedJsonFile.getName() + " and "
									+ expectedJsonFile.getName() + " do not match!");
						}
					} catch (IOException e) {
						e.printStackTrace();
					}
	...
	public static void performAssertions(String testCase, String analysis) {
		Collection<File> generated = CommonOptions.findJimpleFiles(testCase, analysis, PrinterType.GENERATED);
		Collection<File> expected = CommonOptions.findJimpleFiles(testCase, analysis, PrinterType.EXPECTED);

		// Assert number of generates vs expected classes
		if (generated.size() != expected.size()) {
			throw new AssertionError("Number of expected classes = " + expected.size() + " and generated classes = "
					+ generated.size() + ". They should match!");
		}

		// Now diff each generated file with its corresponding expected one,
		// ignoring the xta.XTA and rta.RTA files for now
		generated.stream().filter(f -> !f.getPath().endsWith(xtaJimple) && !f.getPath().endsWith(rtaJimple))
				.forEach(g -> {
					File e = expected.stream().filter(f -> f.getName().equals(g.getName())).findFirst().get();
					try {
	...
	private static void runExpectedOutputPrinter(String testCase, String analysis) {
		...
		averroes.frameworks.ExpectedOutputPrinter
				.main(new String[] { "-i", CommonOptions.getOutputProject(testCase, analysis), "-o",
						CommonOptions.output, "-j", CommonOptions.jre, "-a", analysis });
	}
	...
	private static void runAnalysis(String testCase, String analysis, boolean guard, boolean whole) {
		...
		ArrayList<String> args = new ArrayList<String>(Arrays.asList("-i",
				CommonOptions.getInputProject(testCase), "-o",
				CommonOptions.output, "-j", CommonOptions.jre, "-a", analysis));
