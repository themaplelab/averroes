...
public class UnnecessaryMethodCallEliminator {
	...
	/**
	 * Eliminate unnecessary method calls (e.g., calls to access$0 methods).
	 * This makes text-based diff of Jimple files much easier.
	 * 
	 * @param body
	 */
	public static void transform(Body body) {
		...
		List<Unit> toRemove = body.getUnits().stream()
				.filter(u -> (u instanceof DefinitionStmt && ((DefinitionStmt) u).getRightOp() instanceof InvokeExpr
						&& isRemovable((InvokeExpr) ((DefinitionStmt) u).getRightOp()))
						|| (u instanceof InvokeStmt && isRemovable(((InvokeStmt) u).getInvokeExpr())))
